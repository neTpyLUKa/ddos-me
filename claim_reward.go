...
func main() {
	if len(os.Args) < 4 {
		log.Fatal("Usage: go run submit.go ContractAddress JobAddress PrivateKey")
	}
	...
	privateKey, err := crypto.HexToECDSA(os.Args[3])
	if err != nil {
		log.Fatal(err)
	}
	...
	tx, err := contract.ClaimReward(auth, common.HexToAddress(os.Args[2]))
	if err != nil {
		log.Fatal(err)
	}
